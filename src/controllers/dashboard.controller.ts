...
export const getDashboardStats = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const debts = await prisma.debt.findMany({ where: { userId, status: 'UNPAID' } });
    const payables = debts.filter((d: any) => d.type === 'PAYABLE').reduce((sum: number, d: any) => sum + d.amount, 0);
    const receivables = debts.filter((d: any) => d.type === 'RECEIVABLE').reduce((sum: number, d: any) => sum + d.amount, 0);
    ...
    if (payables === 0) healthScore += 10;
    if (receivables > 0) healthScore += 5;
    if (totalCash > payables) healthScore += 15;
    ...
    const categoryIds = expenseByCategory.map((e: any) => e.categoryId);
    const categories = await prisma.category.findMany({ where: { id: { in: categoryIds } } });
    
    const expenseChartData = expenseByCategory.map((e: any) => {
    ...
    const transactionsForTrend = await prisma.transaction.findMany({
    ...
    transactionsForTrend.forEach((t: any) => {
        const dateStr = t.date.toISOString().split('T')[0];
        if (!dailyNet[dateStr]) dailyNet[dateStr] = 0;
        if (t.type === 'INCOME') dailyNet[dateStr] += t.amount;
        else if (t.type === 'EXPENSE') dailyNet[dateStr] -= t.amount;
    });
    ...
    const totalCashHistory: { date: string; totalCash: number }[] = sortedDates.map(date => {
